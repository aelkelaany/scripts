DECLARE
   ...
   l_pidm          NUMBER;
   cursor get_crns is SELECT DECODE (
           scbcrse_coll_code,
           '00', (SELECT COLL_CODE
 ...
 WHERE     A.SCBCRSE_EFF_TERM =
   ...
   l_dean_pidm number ;
BEGIN
delete from GAC_CRN where term_code='144220';
   items_t (1) := 'TERM';
   items_t (2) := 'CRN';
   items_t (3) := 'COLLEGE';
   items_t (4) := 'DEPARTMENT';
for rec in get_crns loop 
   items_value_t (1) := '144220';
  ...
  -- DBMS_OUTPUT.put_line ('Vice Dean = ' ||  f_get_std_name(l_pidm));
   ...
   insert into GAC_CRN values ('144220',rec.ssbsect_crn,rec.scbcrse_coll_code , rec.scbcrse_dept_code,rec.SCBCRSE_SUBJ_CODE ,rec.SCBCRSE_CRSE_NUMB,rec.scbcrse_title,l_dept_pidm,l_vice_pidm,l_dean_pidm,0
   ,rec.campus ,rec.pterm
   );
