    ...
    FROM (SELECT m.request_no,
                 ...
                 (SELECT ROUND (shrlgpa_gpa, 2)
                   ...
                   WHERE     shrlgpa_pidm = m.requester_pidm
                         AND shrlgpa_levl_code = sgbstdn_levl_code
                         AND shrlgpa_gpa_type_ind = 'O')
                    CGPA ,(select SORTEST_TEST_SCORE from SORTEST where SORTEST_TESC_CODE='S3RA' and SORTEST_PIDM=b.sgbstdn_pidm)as "ËáÇËíÉ" ,(select SORHSCH_GPA from SORHSCH where SORHSCH_pidm=b.sgbstdn_pidm) as  "ËÇäæíÉ"
                    ,(SELECT action_code
