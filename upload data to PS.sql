...
(  SELECT SFRSTCR_PIDM
  ...
  Insert into GLBSLCT
   (GLBSLCT_APPLICATION, GLBSLCT_SELECTION, GLBSLCT_CREATOR_ID, GLBSLCT_DESC, GLBSLCT_LOCK_IND, 
    GLBSLCT_ACTIVITY_DATE, GLBSLCT_TYPE_IND)
 Values
   ('STUDENT', 'COMPARCH', 'SAISUSR', '3302„ÿ«»ﬁ«  ', 'N', 
    SYSDATE, NULL);
 
 
Insert into GLBEXTR
   SELECT 'STUDENT', 'COMPARCH', 'SAISUSR', 'SAISUSR', SGBSTDN_PIDM, 
    SYSDATE, 'S', NULL  FROM 
(  SELECT A.SGBSTDN_PIDM  
    FROM SGBSTDN A 
   WHERE     SGBSTDN_TERM_CODE_EFF =
  ...
  AND A.SGBSTDN_STST_CODE NOT IN ('ŒÃ','„”','ÿ‰')
         ...
         AND A.SGBSTDN_PROGRAM_1 IN ('1M32ARCH38')
 )
  ;
  ...
  Insert into GLBSLCT
   (GLBSLCT_APPLICATION, GLBSLCT_SELECTION, GLBSLCT_CREATOR_ID, GLBSLCT_DESC, GLBSLCT_LOCK_IND, 
    GLBSLCT_ACTIVITY_DATE, GLBSLCT_TYPE_IND)
 Values
   ('STUDENT', 'COMP44', 'SAISUSR', 'COMP44', 'N', 
    SYSDATE, NULL);
 
 
Insert into GLBEXTR
   SELECT 'STUDENT', 'COMP44', 'SAISUSR', 'SAISUSR', SGBSTDN_PIDM, 
    SYSDATE, 'S', NULL  FROM 
(  SELECT A.SGBSTDN_PIDM  
    FROM SGBSTDN A 
   WHERE     SGBSTDN_TERM_CODE_EFF =
...
(  SELECT A.SGBSTDN_PIDM  
   ...
   WHERE     SGBSTDN_TERM_CODE_EFF =
                (SELECT MAX (B.SGBSTDN_TERM_CODE_EFF)
                  ...
                  WHERE     
  B.SGBSTDN_PIDM = A.SGBSTDN_PIDM)
 AND A.SGBSTDN_STST_CODE   IN ('ŒÃ','„”','ÿ‰')
