...
DECLARE
   ...
   IS
      SELECT DISTINCT
       SCRRTST_SUBJ_CODE_PREQ || SCRRTST_CRSE_NUMB_PREQ wrong,
       SCBCRSE_TITLE wrong_TITLE
  FROM SCRRTST s, SCBCRSE
 WHERE     SCRRTST_SUBJ_CODE || SCRRTST_CRSE_NUMB = M_COURSE
     and SCBCRSE_CSTA_CODE='A'
       AND scbcrse_SUBJ_CODE || scbcrse_CRSE_NUMB =
             SCRRTST_SUBJ_CODE_PREQ || SCRRTST_CRSE_NUMB_PREQ
              ...
              (SELECT '1'
                ...
                WHERE COURSE_SUBJ_CODE || COURSE_CRSE_NUMB = P_COURSE
                  AND TERM_CODE_EFF IN ('143810'))
                   and not exists (select '2' from SCRRTST where SCRRTST_SUBJ_CODE || SCRRTST_CRSE_NUMB=
                s.SCRRTST_SUBJ_CODE || s.SCRRTST_CRSE_NUMB
                and  SCRRTST_CONNECTOR='A')
                
                ;

BEGIN
   DBMS_OUTPUT.put_line ('MAIN COURSE ,MAIN TITLE, PREQ COURSE ,PREQ TITLE, WRONG ,WRONG_TITLE');
   ...
   LOOP
      ...
      LOOP
         /*DBMS_OUTPUT.put_line (
            ...
            || Q.MISSING);*/
         DBMS_OUTPUT.put_line (
            M.MAIN_COURSE || ' ,'|| M.MAIN_TITLE || ' ,'|| M.PREQ_COURSE || ' ,' ||M.PREQ_TITLE||' ,' || Q.wrong ||' ,'|| Q.wrong_TITLE);
      END LOOP;
   END LOOP;
END;