/* Formatted on 1/8/2022 11:29:42 AM (QP5 v5.365) */
--UGR1
          ...
          -- AND SGBSTDN_COLL_CODE_1='12'
           ...
           AND NOT EXISTS
                   (SELECT '1'
                     ...
                     and SFBRGRP_TERM_CODE='144320')
--           AND NOT EXISTS
--                   (SELECT '1'
--                      FROM SMBPOGN
--                     WHERE     SMBPOGN_MET_IND = 'Y'
--                           AND SMBPOGN_pidm = SG.sgbstdn_pidm
--                           AND EXISTS
--                                   (SELECT '1'
--                                      FROM shrlgpa
--                                     WHERE     SHRLGPA_pidm = SG.sgbstdn_pidm
--                                           AND SHRLGPA_GPA_TYPE_IND = 'O'
--                                           AND SHRLGPA_GPA > 1))
;

-- Release Registration Hold
    ...
    CURSOR get_data IS
        SELECT a1.sgbstdn_pidm                      student_pidm,
         ...
         WHERE     a1.sgbstdn_term_code_eff =
               ...
               AND (   (    a1.sgbstdn_levl_code IN ('MA', 'œ⁄')
                        AND c1.smbpogn_req_credits_overall >= 30)
                    OR (    a1.sgbstdn_levl_code IN ('œ»')
                        AND c1.smbpogn_req_credits_overall >= 60)
                    OR (    a1.sgbstdn_levl_code IN ('Ã„', 'Ã5')
...
BEGIN
    ...
    LOOP
        ...
        UPDATE sfbetrm
         ...
         WHERE sfbetrm_pidm = i.student_pidm AND sfbetrm_term_code = '144320';
    ...
    UPDATE sfbetrm
     ...
     WHERE sfbetrm_term_code = '144320' AND sfbetrm_mhrs_over > 24;
