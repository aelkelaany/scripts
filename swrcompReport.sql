SELECT  
decode(comp.met_ind,'Y','„ Õﬁﬁ','€Ì— „ Õﬁﬁ') comp_met_ind,decode(comp.area_MET_IND,'Y','„ Õﬁﬁ','€Ì— „ Õﬁﬁ') area_met_ind_desc ,
decode(REQUIRED_FLAG,'Y','„ ÿ·»','N','≈Œ Ì«—Ì') elective_section ,
AREA_PRIORITY,
         capp.AREA_CODE,
   ...
   AND a.scbcrse_crse_numb =capp.course_number
    ...
    FROM scbcrse
     ...
     AND scbcrse_eff_term <=CAPP.EFFECTIVE_TERM)) subj_crd_hrs ,
      ...
      F_GET_DESC_FNC ('STVMAJR', comp.MAJR_CODE, 60) MAJR_DESC ,    F_GET_DESC_FNC ('STVSTYP', comp.sgbstdn_styp_code, 60) styp_DESC ,    comp.TERM_CODE_CATLG , F_GET_DESC_FNC ('STVSTST', comp.sgbstdn_stst_code, 60) stst_DESC,
      COMP.*                  
...
SMBPOGN_PIDM                                       PIDM,
       ...
       ADJ.SMRDOUS_SUBJ_CODE || ADJ.SMRDOUS_CRSE_NUMB     ADJ_SUBJ ,P.SMRDOUS_CREDIT_HOURS_USED,SGBSTDN_STST_CODE
  FROM SMBPOGN,
       sgbstdn,spriden  ,
       SMBAOGN,
       SMRDORQ,
...
SMBPOGN_PIDM                                       PIDM,
 ...
 WHERE     SGBSTDN_PIDM = SMBPOGN_PIDM
      ...
      AND CAPP.COURSE_NUMBER    = COMP.CRSE_NUMB         
      -- prarameters
 
where comp.student_id  BETWEEN   :P_id_FROM AND :P_ID_TO
   /* AND NVL (comp.sgbstdn_styp_code, '%') LIKE :P_styp
and nvl(comp.sgbstdn_stst_code,'%' )like :p_stst
 AND NVL (comp.COLL_CODE, '%') LIKE :p_cOLL
         AND NVL (comp.LEVL_CODE, '%') LIKE :P_LEVL
         AND NVL (comp.PROGRAM_code, '%') LIKE :P_PROG
         AND NVL (comp.DEPT_CODE, '%') LIKE :P_DEPT
         AND prog_ACT_CREDITS_OVERALL  BETWEEN NVL (:P_HRS_MIN, 0)
