CREATE OR REPLACE PROCEDURE BU_APPS.p_update_std_status (
   pidm           NUMBER,
   term           VARCHAR2,
   status_code    VARCHAR2,
   data_origin    VARCHAR2 DEFAULT NULL)
IS
BEGIN
   UPDATE sgbstdn
      SET sgbstdn_stst_code = status_code,
          SGBSTDN_ACTIVITY_DATE = SYSDATE,
          SGBSTDN_DATA_ORIGIN = data_origin,
          sgbstdn_user_id = USER
    WHERE sgbstdn_pidm = pidm AND sgbstdn_term_code_eff = term;
