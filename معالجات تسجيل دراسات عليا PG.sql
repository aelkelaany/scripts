...
--CRN>>COL11
-- pidm.col01
update BU_DEV.TMP_TBL_kilany set col01=f_get_pidm(col02)  ;
...
INSERT INTO STVBLCK
      SELECT DISTINCT 'PG' || COL11, 'Post Graduate Students ' || COL11, SYSDATE
        FROM BU_DEV.TMP_TBL_kilany
    ORDER BY 1;
...
DECLARE
    ...
    CURSOR GET_DATA IS
        SELECT 'PG' || COL11 BLCK, COL01 PIDM
          FROM BU_DEV.TMP_TBL_kilany
         WHERE col01 != 0;
BEGIN
    FOR REC IN GET_DATA
...
-- still records loop again 
delete from BU_DEV.TMP_TBL_kilany A
 WHERE    EXISTS
           (SELECT '1'
              FROM sfrstcr
             WHERE     sfrstcr_term_code = '144510'
