      ...
      WHERE     EXISTS
            ...
            AND EXISTS
                   (SELECT '2'
                     ...
                     WHERE shrtrce_pidm = std_pidm AND process_id = 2200);
...
DELETE FROM SHRTRCR
      WHERE     EXISTS
            ...
            AND EXISTS
                   (SELECT '2'
                      FROM CAPP_MNPL_LOG_DETAIL
                     WHERE SHRTRCR_pidm = std_pidm AND process_id = 2200);
...
DELETE FROM SHRTRAM
      WHERE     EXISTS
            ...
            AND EXISTS
                   (SELECT '2'
                      FROM CAPP_MNPL_LOG_DETAIL
                     WHERE SHRTRAM_pidm = std_pidm AND process_id = 2200);
...
DELETE FROM SHRTRIT
      WHERE     SHRTRIT_SBGI_CODE = '99'
            AND EXISTS
                   (SELECT '2'
                      FROM CAPP_MNPL_LOG_DETAIL
                     WHERE SHRTRIT_pidm = std_pidm AND process_id = 2200)