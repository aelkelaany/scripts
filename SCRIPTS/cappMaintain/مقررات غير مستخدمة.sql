/* Formatted on 14/04/2021 13:26:21 (QP5 v5.227.12220.39754) */
SELECT f_get_std_id (SMRDOCN_PIDM) std_id,
  ...
  FROM SATURN.SMRDOCN m, smrprle, SGRADVR
...
--                      AND SHRGRDE_PASSED_IND = 'Y')
       ...
       AND F_GET_STYP (SMRDOCN_PIDM) = 'ã'
       AND SMRPRLE_PROGRAM = SMRDOCN_PROGRAM
       AND SMRDOCN_LEVL_CODE = 'Ìã'
       AND SUBSTR (f_get_std_id (SMRDOCN_PIDM), 1, 3) >= '433'
       AND SUBSTR (f_get_std_id (SMRDOCN_PIDM), 1, 3) < '438'
       AND EXISTS
              (SELECT '1'
