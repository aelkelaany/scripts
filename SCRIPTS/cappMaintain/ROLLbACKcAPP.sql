/* Formatted on 23/12/2020 14:50:40 (QP5 v5.227.12220.39754) */
DELETE FROM SATURN.SMRSSUB
      WHERE EXISTS
               (SELECT '1'
                  FROM CAPP_MNPL_LOG_DETAIL
                 WHERE     PROCESS_ID = '1600'
...
DELETE FROM SHRTRCR
      WHERE     EXISTS
                   (SELECT '1'
                      FROM CAPP_MNPL_LOG_DETAIL
                     WHERE PROCESS_ID = '1600' AND STD_PIDM = SHRTRCR_PIDM)
...
DELETE FROM SHRTRAM
      WHERE     EXISTS
                   (SELECT '1'
                      FROM CAPP_MNPL_LOG_DETAIL
                     WHERE PROCESS_ID = '1600' AND STD_PIDM = SHRTRAM_PIDM)
...
DELETE FROM SHRTRIT
      WHERE     (SHRTRIT_PIDM, SHRTRIT_SEQ_NO) NOT IN
            ...
            AND EXISTS
                   (SELECT '1'
                      FROM CAPP_MNPL_LOG_DETAIL
                     WHERE PROCESS_ID = '1600' AND STD_PIDM = SHRTRIT_PIDM)