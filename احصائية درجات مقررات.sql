  ...
  (select DISTINCT
         ...
         f_get_desc_fnc ('STVCOLL', scbcrse_coll_code, 60) college_desc,f_get_desc_fnc ('STVDEPT', scbcrse_dept_code, 60) department_desc ,scbcrse_subj_code || scbcrse_crse_numb course_number,
   ...
   WHERE     sfrstcr_term_code = ssbsect_term_code
         ...
         GROUP BY scbcrse_coll_code,scbcrse_dept_code ,scbcrse_subj_code ,scbcrse_crse_numb ,scbcrse_title ,shrgrds_grde_code_substitute

)
 

  PIVOT  ( sum(count_over_college)  AS "Count" FOR (grade) IN ('Γ+' AS  "A+" ,'Γ' AS "A",'Θ+' AS "B+",'Θ' AS "B",'Μ+' AS "C+",'Μ' AS "C",'Ο+' AS "D+",'Ο' AS "D",'εά' AS "F" )) 
 ORDER BY 1,2, 3
