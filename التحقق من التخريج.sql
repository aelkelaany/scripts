...
Insert into GLBSLCT
 ...
 Values
   ('STUDENT', 'GRD_DP3_23', 'SAISUSR', 'œ»·Ê„ ⁄«·Ì23 ', 'N', 
    SYSDATE, NULL);
 

Insert into GLBEXTR
   SELECT 'STUDENT', 'GRD_DP3_23', 'SAISUSR', 'SAISUSR', PIDM, 
    SYSDATE, 'S', NULL  FROM 
...
SELECT F_GET_STD_ID(SGBSTDN_PIDM) SSID ,F_GET_STD_NAME(SGBSTDN_PIDM) SNAME 
FROM SGBSTDN SG
WHERE SGBSTDN_PIDM IN 

    (  SELECT   GLBEXTR_KEY PIDM
FROM GLBEXTR
where GLBEXTR_SELECTION='GRD_33_14430' 
...
UNION
  ...
  SELECT F_GET_STD_ID(shrdgmr_pidm) SSID ,F_GET_STD_NAME(shrdgmr_pidm) SNAME  --, SHRDGMR_LEVL_CODE ,shrdgmr_pidm
FROM 
shrdgmr A
