...
SELECT *
  FROM (  SELECT faculty_id,
                 ...
                 SUM (contact),
                 sirasgn_pidm
            FROM (SELECT DISTINCT F_GET_DESC_FNC ('STVCOLL', SIRDPCL_COLL_CODE, 60)
                            COLL_DESC,
                         sirasgn_pidm,
       ...
       (  SELECT COUNT (crn),
                 ...
                 SUM (contact),
                 sirasgn_pidm
            FROM (SELECT DISTINCT sirasgn_pidm,
                         ssbsect_crn crn,
                         GREATEST (NVL (a.SCBCRSE_CREDIT_HR_LOW, 0),
                                   NVL (a.SCBCRSE_CREDIT_HR_HIGH, 0))
                            credit,
 ...
 WHERE a.sirasgn_pidm = b.sirasgn_pidm(+)
 order by 1